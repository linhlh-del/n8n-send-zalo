  ...
  try {
    ...
    if (queueSheet && queueSheet.getLastRow() > 1) {
      ...
      data.forEach(row => {
        ...
        else if (status === "Success" || status.includes("Success")) queueStats.sent++;
...
// ==========================================
// LƯU Ý: N8N TỰ ĐỘNG CẬP NHẬT STATUS
// ==========================================
// Workflow n8n sẽ tự động cập nhật cột L (Status) thành "Success" 
// khi gửi tin nhắn Zalo thành công, nên không cần giai đoạn đồng bộ riêng.
...
function onOpen() {
  ...
  logSystem("Menu khởi tạo - n8n tự động cập nhật Status");
}
