  ...
  try {
    ...
    return {
      ...
      message: '✅ Chuyển lead thành công!\n\n' +
              ...
              '📤 Từ: ' + fromRVA.name + '\n' +
              '📥 Sang: ' + toRVA.name + '\n\n' +
              '💬 Tin nhắn có context đã được thêm vào Queue.\n' +
              '🚀 Chạy Giai đoạn 3 để gửi cho RVA mới.'
    };
...
// ==========================================
// 🔄 ĐỒNG BỘ STATUS VỀ LEAD-MKT (v2)
// ==========================================
function syncStatusToLeadSheetV2() {
  try {
    logSystem("🔄 Bắt đầu đồng bộ Status & Time từ Queue về lead-mkt (v2)");
    ...
    queueData.forEach(function(row) {
      ...
      if (leadId && rvaId && status) {
        let statusToWrite = "";
        let careStatus = "Chưa liên hệ";
        
        if (status.toLowerCase() === "pending") {
          statusToWrite = "Pending";
          careStatus = "Chưa liên hệ";
    ...
    logSystem('✅ Đồng bộ v2 hoàn tất: ' + updatedCount + ' lead được cập nhật', "SUCCESS");
    
    return {
      success: true,
      message: '✅ Đồng bộ hoàn tất!\n\n' +
              '📊 Đã cập nhật: ' + updatedCount + ' lead\n' +
              '(Bao gồm: Status, Thời gian, Trạng thái chăm sóc)'
    };
