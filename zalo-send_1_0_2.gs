...
const SYSTEM_CONFIG = {
  ...
  N8N_CONFIG: {
    ...
    RESULT_COL: 18         // Cập nhật từ 17 -> 18
  }
};
...
function getSystemStats() {
  try {
    ...
    if (leadSheet && leadSheet.getLastRow() >= SYSTEM_CONFIG.LEADS_CONFIG.START_ROW) {
      const lastRow = leadSheet.getLastRow();
      const startRow = SYSTEM_CONFIG.LEADS_CONFIG.START_ROW;
      const data = leadSheet.getRange(startRow, 1, lastRow - startRow + 1, 10).getValues();
      
      data.forEach(row => {
        const fullName = row[2]; // COL_NAME = 3 (index 2)
        if (fullName && fullName !== "") {
          leadStats.total++;
          
          const rva1 = row[7]; // COL_RVA1 = 8 (index 7)
          const rva2 = row[8]; // COL_RVA2 = 9 (index 8)
          const rva3 = row[9]; // COL_RVA3 = 10 (index 9)
...
function runStage1FromUI() {
  try {
    ...
    data.forEach(function(row) {
      const fullName = row[2]; // COL_NAME = 3 (index 2)
      const leadId = row[1];   // COL_LEAD_ID = 2 (index 1)
...
function runStage2FromUI() {
  try {
    ...
    // Lấy danh sách Lead ID đã gửi từ Queue
    const queueSheet = ss.getSheetByName(SYSTEM_CONFIG.SHEETS.QUEUE);
    ...
    if (queueSheet && queueSheet.getLastRow() > 1) {
      ...
      logSystem('Đã tìm thấy ' + sentLeadIds.size + ' Lead ID trong Queue');
    }
    ...
    data.forEach(function(row, index) {
      ...
      if (fullName && fullName !== "" && rva1 === "" && rva2 === "" && rva3 === "") {
        ...
        // Kiểm tra trùng lặp
        if (sentLeadIds.has(String(leadId).trim())) {
          skippedDuplicate++;
          logSystem('Bỏ qua Lead ID trùng: ' + leadId, "INFO");
          return;
        }
      ...
      if (responseCode === 200) {
        ...
        return {
          ...
          message: 'Đã kích hoạt n8n thành công!\n\n' +
                  ...
                  'Cột L (Status) sẽ chuyển từ "Pending" sang "Sent"\n' +
                  'Cột Q (Message ID) sẽ hiển thị ID tin nhắn'
        };
...
// ==========================================
// GIAI ĐOẠN 4: ĐỒNG BỘ STATUS TỪ N8N
// ==========================================
function syncStatusFromN8n() {
  try {
    logSystem("Bắt đầu đồng bộ Status từ cột R sang cột L");
    ...
    if (updatesToApply.length > 0) {
      logSystem('Đang cập nhật ' + updatesToApply.length + ' dòng...');
      
      updatesToApply.forEach(function(update) {
        queueSheet.getRange(update.row, statusCol).setValue(update.newStatus);
      });
      
      logSystem('Đã cập nhật ' + updatedCount + ' dòng từ Pending sang Sent', "SUCCESS");
    }
    ...
    return {
      ...
      message: 'Đồng bộ hoàn tất!\n\n' +
              ...
              'Kiểm tra sheet "Zalo_Queue" cột L'
    };
...
// ==========================================
// HÀM TỰ ĐỘNG ĐỒNG BỘ (NÂNG CAO)
// ==========================================
function autoSyncStatusFromN8n() {
  try {
    logSystem("Auto-sync: Bắt đầu kiểm tra và đồng bộ");
    
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const queueSheet = ss.getSheetByName(SYSTEM_CONFIG.SHEETS.QUEUE);
    
    if (!queueSheet || queueSheet.getLastRow() <= 1) {
      logSystem("Auto-sync: Queue trống, bỏ qua");
      return;
    }
    ...
    if (updatedCount > 0) {
      logSystem('Auto-sync: Đã cập nhật ' + updatedCount + ' dòng', "SUCCESS");
    } else {
      logSystem("Auto-sync: Không có gì cần cập nhật");
    }
...
function syncStatusMenu() {
  ...
  const confirm = ui.alert(
    'ĐỒNG BỘ STATUS',
    'Cập nhật Status từ kết quả n8n?\n\nPending sang Sent (nếu cột R có Success)',
    ui.ButtonSet.YES_NO
  );
...
function onOpen() {
  ...
  logSystem("Menu khởi tạo - Phiên bản có Lead ID");
}
